...
// Hitung nilai threshold pada setiap pixel
  for i:= 0 to 63 do
    for j:= 0 to 63 do
    begin
      for k:= 0 to 19 do
        arrThres[k]:= matDiffH[i+(k mod 4),j+(k div 4)];
      for k:= 0 to 19 do
        arrThres[k+20]:= matDiffV[i+(k mod 5),j+(k div 5)];
      // Pengurutan Data
      m:= 1;
      n:= 2;
      while m < 40 do
      ...
      // Ambil nilai median
      matThres[i,j]:= Ceil((arrThres[19]+arrThres[20])/2);
    end;
...
// Hitung Feature Map pada 4 arah
  for i:= 0 to 63 do
    for j:= 0 to 63 do
    begin
      matMapH[i,j]:= abs(sumber[i,j+1] + sumber[i+1,j+1] + sumber[i+2,j+1] + sumber[i+3,j+1] + sumber[i+4,j+1]
      ...
      matMapP45[i,j]:= abs(sumber[i+3,j]
                   + sumber[i+1,j+1] + sumber[i+2,j+1]
                   + sumber[i+1,j+2]
                   + sumber[i,j+3]
                   - sumber[i+4,j+1]
                   - sumber[i+3,j+2]
                   - sumber[i+2,j+3] - sumber[i+3,j+3]
                   - sumber[i+1,j+4]
                   );
      ...
      matMapV[i,j]:= abs(sumber[i+1,j] + sumber[i+1,j+1] + sumber[i+1,j+2] + sumber[i+1,j+3] + sumber[i+1,j+4]
      ...
      matMapM45[i,j]:= abs(sumber[i+1,j]
                   + sumber[i+2,j+1] + sumber[i+3,j+1]
                   + sumber[i+3,j+2]
                   + sumber[i+4,j+3]
                   - sumber[i,j+1]
                   - sumber[i+1,j+2]
                   - sumber[i+1,j+3] - sumber[i+2,j+3]
                   - sumber[i+3,j+4]
                   );
